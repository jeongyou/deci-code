...
**모든 변경**은 예외 없이 이 순서로 진행한다. 기능, 버그, 문서, 설정, CI/CD 변경 모두 동일하다. main에 직접 커밋하지 않는다. "작은 작업"이라도 PLAN을 건너뛰지 않는다.

1. PLAN: `docs/plans/NNN-{설명}.md` (번호 순서)에 목적·범위·작업 내용을 기록하고 승인을 받는다. 완료 후 삭제하지 않고 보관한다. **승인 없이 이슈 생성이나 구현을 시작하지 않는다.**
